/*
    ...
    Morse code is now mostly superseded by voice and digital data 
    ...
    message is written in Morse code, a single space is used to 
    separate the character codes and 3 spaces are used to separate 
    ...
    distress signal SOS (that was first issued by Titanic), that is 
    coded as ···−−−···. These special codes are treated as single 
...
const decodeMorse = (morseCode) => {
  let formated = morseCode.split("   ");
  let message = "";

  for (const setence of formated) {
    let morseCodeArr = setence.split(" ");
    for (const code of morseCodeArr) {
      if (MORSE_CODE[code] !== undefined) {
        message += MORSE_CODE[code];
      }
    }
    message += " ";
  }
